...
std::expected<size_t, io_error> blocking_io::write_all(int fd, const std::string& input_text){
    size_t total = 0;
...
std::expected<std::string, io_error> blocking_io::read_all(int fd){
    std::string output;
    char buf[4096];
...
std::expected<std::string, io_error> blocking_io::read_all_from_start(int fd){
