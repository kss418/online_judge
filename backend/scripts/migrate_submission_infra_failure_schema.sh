...
echo "apply submission_infra_failure_schema"
psql "${database_url}" \
...
BEGIN
    IF NOT EXISTS(
        ...
        RAISE EXCEPTION 'submission_schema must be applied before submission_infra_failure_schema';
    END IF;
    ...
    IF NOT EXISTS(
        ...
        RAISE EXCEPTION 'user_statistics_schema must be applied before submission_infra_failure_schema';
    END IF;
END
$do$;

ALTER TYPE submission_status ADD VALUE IF NOT EXISTS 'infra_failure';

ALTER TABLE user_submission_statistics
    ADD COLUMN IF NOT EXISTS infra_failure_submission_count BIGINT NOT NULL DEFAULT 0;
...
SET infra_failure_submission_count = aggregated.infra_failure_submission_count,
    updated_at = NOW()
FROM (
    SELECT
        submissions.user_id,
...
VALUES('submission_infra_failure_schema_v1')
