...
std::expected<pl_runner::prepared_source, error_code> python_runner::prepare(
    const path& source_file_path,
    const path& python_path
){
    ...
    if(!compile_python_exp->is_success()){
        return pl_runner::make_compile_failed_prepared_source(
            compile_python_exp->exit_code_,
            std::move(compile_python_exp->stderr_text_)
        );
    }

    pl_runner::prepared_source prepared_source_value;
